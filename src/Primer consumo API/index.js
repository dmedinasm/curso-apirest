...
api.defaults.headers.common["X-API-KEY"] =
  "live_Je2iJayLCJmORfO2DRBgtpj4HhyrRmjiiyeLO9MtQVkxryEFEsjd85f7xWVIzB2s";
...
function fetching(url_endpoint, ini = {}) {
  const request = new Request(url_endpoint, ini);
  let xhttp = new XMLHttpRequest();
  xhttp.open(request.method, url_endpoint);
  if (ini.headers) {
    const heads = Object.entries(ini.headers);
    console.log(heads);
    for (let values of heads) {
      xhttp.setRequestHeader(values[0], values[1]);
    }
  }
  xhttp.send(ini.body);
  return new Promise((resolve, reject) => {
    xhttp.onload = function () {
      resolve(new Response(xhttp.response));
    };
    xhttp.onerror = (err) => {
...
async function loadDogsRandom() {
  const response = await fetching(API_URL_RANDOM);
...
async function saveFavoritesDogs(id) {
  const res = await fetching(API_URL_FAVOURITES, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-API-KEY":
  ...
  console.log(res);
  console.log("favoritesClon");
}

loadDogsRandom();
saveFavoritesDogs("HkC31gcNm");
