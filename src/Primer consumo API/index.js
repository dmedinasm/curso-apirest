const API_URL_RANDOM = "https://api.thedogapi.com/v1/images/search?limit=2&api_key=live_Je2iJayLCJmORfO2DRBgtpj4HhyrRmjiiyeLO9MtQVkxryEFEsjd85f7xWVIzB2s";
...
async function loadFavoritesDogs() {
  ...
  if (response.status !== 200) {
    divError.innerHTML = "Hubo un error: " + response.status + " " + compareCodes(response.status).message;
  } else {
    const data = await response.json();
